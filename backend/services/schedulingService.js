...
async function updatePendingPropertySchedule(connection, propertyId, estimateId, franchisePartnerId) {
  ...
  // Upsert pending_property_schedules - use 'status' column (not 'scheduling_status')
  // Map status values to match table ENUM
  const mappedStatus = schedulingStatus === 'pending_vendor' ? 'pending_vendor_assignment' 
    : schedulingStatus === 'pending_schedule' ? 'pending_scheduling' 
    : schedulingStatus;
  ...
  await connection.execute(
    ...
    [propertyId, estimateId, totalServices, assignedCount, scheduledCount, 
     mappedStatus, franchisePartnerId]
  );
}
...
async function getPendingSchedulesCount(franchisePartnerId) {
  try {
    let query = `
      ...
      WHERE pps.status IN ('pending_vendor_assignment', 'partial_vendor_assignment', 'pending_scheduling')
    `;
    const params = [];
