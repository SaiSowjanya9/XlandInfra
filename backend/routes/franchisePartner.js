...
router.get('/dashboard', requireFPScope, async (req, res) => {
  try {
    ...
    const [
      ...
      // Database uses codes like 'GC', 'APT', 'AP', 'VILLA', 'VL', 'FLAT', 'FL', 'PLOT', 'PL'
      pool.execute(`
        SELECT 
          ...
          SUM(CASE WHEN estimate_type = 'direct' AND (property_type IS NULL OR property_type = '' OR (
            UPPER(property_type) NOT IN ('GC', 'APT', 'AP', 'VILLA', 'VL', 'FLAT', 'FL', 'PLOT', 'PL') AND
    ...
    if (status === 'completed') {
      ...
      if (workOrder.length > 0) {
        ...
        try {
          ...
          console.log('[FP] Completion email sent');
        } catch (err) {
          console.error('[FP] Completion email error:', err);
        }
      }
...
router.get('/estimates', requireFPScope, async (req, res) => {
  try {
    ...
    let query = `SELECT fe.*, 
                        ...
                        COALESCE(NULLIF(fe.amc_package_description, ''), fpamc_id.description, fpamc_name.description) as amc_package_description,
    ...
    if (property_id) {
      ...
      if (isNaN(numericId)) {
        ...
        query += ' AND fe.property_code = ?';
        params.push(property_id);
      } else {
        // Numeric property ID
        query += ' AND (fe.property_code = ? OR fe.property_id = ?)';
        params.push(property_id, numericId);
      }
    }
...
router.put('/estimates/:id/status', requireFPScope, async (req, res) => {
  try {
    ...
    if (normalizedStatus === 'approved') {
      console.log(`🔔 Estimate ${req.params.id} approved - starting auto-invoice generation`);
      try {
        const { generateInvoiceFromEstimate } = require('../services/invoiceService');
        // Use 'fp' source since this is an fp_estimates record
        invoiceResult = await generateInvoiceFromEstimate(req.params.id, req.user?.id || null, 'fp');
        console.log(`✅ Auto-generated invoice for approved estimate:`, invoiceResult);
      } catch (invoiceError) {
    ...
    res.json({ 
      success: true, 
      message: invoiceResult?.invoiceId 
        ? `Status updated to approved and invoice ${invoiceResult.invoiceId} created` 
        : 'Status updated successfully', 
      status: normalizedStatus,
      invoiceCreated: !!invoiceResult?.invoiceId,
...
router.put('/estimates/:id', requireFPScope, async (req, res) => {
  try {
    ...
    // Verify estimate belongs to this FP
    const [[existing]] = await pool.execute(
...
router.get('/services', authenticate, attachFPScope, async (req, res) => {
  try {
    ...
    // Add common services if none found
    if (serviceMap.size === 0) {
      ['Pest Control', 'Deep Cleaning', 'AC Service', 'Plumbing', 'Electrical', 'HVAC', 'Lift AMC'].forEach(name => {
        serviceMap.set(name, { id: name, name, category: name });
      });
    }
...
// Get all service types (global + FP-specific)
router.get('/service-types', requireFPScope, async (req, res) => {
  try {
    const [rows] = await pool.execute(
      `SELECT id, name, is_global, franchise_partner_id, created_at 
       FROM service_types 
       WHERE is_active = 1 AND (is_global = 1 OR franchise_partner_id = ?)
       ORDER BY name ASC`,
      [req.fpId]
    );
...
// Add new service type (FP-specific)
router.post('/service-types', requireFPScope, async (req, res) => {
  try {
    const { name } = req.body;
    if (!name?.trim()) {
...
// Delete service type (FP can only delete their own)
router.delete('/service-types/:id', requireFPScope, async (req, res) => {
  try {
    const { id } = req.params;
    await pool.execute(
      'UPDATE service_types SET is_active = 0 WHERE id = ? AND franchise_partner_id = ?', 
      [id, req.fpId]
    );
    ...
    const query = `
      SELECT * FROM (
        ...
        SELECT DISTINCT
          ...
          COALESCE(fe.package_services, fe.service_rows, fpamc.services) as serviceRows,
...
router.get('/schedules/pending-count', async (req, res) => {
  try {
    const count = await schedulingService.getPendingSchedulesCount(req.fpId);
...
router.get('/schedules/all', authenticate, attachFPScope, async (req, res) => {
  try {
    ...
    try {
      ...
      console.log('[FP All Schedules] Count query failed:', countErr.message);
    }
    ...
    const query = `
      SELECT 
        ...
        op.property_type as propertyType,
        op.zone,
        COALESCE(pc.name, op.contact_person) as customerName,
        COALESCE(pc.phone, op.contact_phone) as customerPhone,
        COALESCE(pc.email, op.contact_email) as customerEmail,
        ov.id as vendorDbId,
...
router.post('/schedules/assign-vendor', async (req, res) => {
  try {
...
router.get('/schedules/notifications', async (req, res) => {
  try {
...
router.put('/schedules/notifications/:notificationId/read', async (req, res) => {
  try {
