...
router.get('/estimates', requireFPScope, async (req, res) => {
  try {
    ...
    let query = `SELECT fe.*, 
                 ...
                 LEFT JOIN onboarded_properties op ON (fe.property_id = op.id OR fe.property_code = op.property_id OR CAST(fe.property_id AS CHAR) = op.property_id)
                 LEFT JOIN properties p ON (fe.property_id = p.id OR fe.property_code = p.property_id OR CAST(fe.property_id AS CHAR) = p.property_id) AND op.id IS NULL
