...
router.get('/pending-properties', authenticate, canSeeSchedule, async (req, res) => {
  try {
    ...
    // Uses UNION to support both onboarded_properties and legacy properties tables
    ...
    let query = `
      SELECT * FROM (
        ...
        -- From properties (legacy table) - for backward compatibility
        SELECT 
          p.id,
  ...
  try {
    ...
    const [estimates] = await pool.execute(
      `SELECT 
       ...
       LEFT JOIN fp_amc_packages fpamc ON fpamc.id = fe.package_id
       WHERE fe.property_id = ? AND fe.payment_status IN ('paid', 'partial')
...
router.get('/all', authenticate, canSeeSchedule, async (req, res) => {
  try {
    ...
    try {
      ...
      console.log('[All Schedules] Count query failed:', countErr.message);
    }
