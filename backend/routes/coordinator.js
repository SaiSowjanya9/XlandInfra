  ...
  try {
    ...
    const [onboardedPropsCount] = await pool.query(
      `SELECT COUNT(*) as count FROM onboarded_properties op
       WHERE (op.franchise_partner_id = ? AND op.status = 'active' AND (op.created_by = ? OR op.created_by = ? OR op.coordinator_id = ?${onbZoneCondition}))`,
    ...
    const [recentWorkOrders] = await pool.query(
      `SELECT wo.*, p.name as property_name, COALESCE(c.name, wo.category_name) as category_name,
       ...
       WHERE wo.franchise_partner_id = ? AND (wo.created_by = ? OR wo.created_by = ? OR wo.coordinator_id = ?${zonePlaceholders ? ` OR p.zone_id IN (${zonePlaceholders})` : ''})
       ORDER BY wo.created_at DESC
       LIMIT 10`,
      [franchisePartnerId, creatorEmail, req.user?.username || '', coordinatorId, ...assignedZones]
    );
