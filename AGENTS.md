...
- Service form fields and toggles carry no helper text under them: labels stand alone, and `Field`/`Toggle` accept no `hint`. The blue example-pricing disclaimers are gone too — they described a configured operating cost that no longer exists. The distinction between an example and final estimate pricing is carried by the wording instead: the heading reads "Pricing Preview (Example)" and the total "Example Customer Price" **only when an example amount is in play** (area, quantity, capacity, slab capacity or manpower). A Fixed Price preview assumes nothing — rate × visits is the real annual figure — so it drops the word and reads "Pricing Preview" and "Customer Price".
