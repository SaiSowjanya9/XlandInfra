...
- Service form fields and toggles carry no helper text under them: labels stand alone, and `Field`/`Toggle` accept no `hint`. The example-pricing disclaimers stay, because example pricing must remain clearly distinguishable from final estimate pricing.
...
- A service configures the money once and the estimate inherits it: the vendor rate per method, `default_operating_cost` (XLAND's annual operating cost) and `default_markup_percentage`. `calculateServiceQuote` falls back to `input.operating_cost ?? config.default_operating_cost ?? 0`, so an estimate needs to send nothing to get the right figures, and may still override the operating cost per estimate. `ServiceCatalogPicker` quotes on the server as the inputs change (400ms debounce) and shows Vendor Cost, XLAND Operating Cost, Actual Cost, Markup, Customer Price and Margin, which is what makes those values appear by themselves. Every preview, including `previewManpower`, applies markup to the actual cost (vendor + operating), never to the vendor cost alone — a test asserts the frontend preview equals the backend quote with a non-zero operating cost.
