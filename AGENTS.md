...
- **XLAND's cost is the markup, not a separate figure.** A vendor rate of ₹1,000 a visit at 35% earns ₹350, and the customer pays ₹1,350: `XLAND Cost = vendor cost × markup %` and `Customer Price = vendor cost + XLAND Cost`. So a service configures only the vendor rate per method and `default_markup_percentage`; the Add Service form has no operating-cost input and always saves `default_operating_cost: 0`, showing a read-only XLAND Cost derived from the rate and the markup instead. Capacity Slab shows no XLAND field at all, since it prices from its slabs. The Configured Services table derives its XLAND Cost column the same way, by scaling the vendor rate.
