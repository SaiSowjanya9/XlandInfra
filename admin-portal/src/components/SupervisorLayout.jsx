...
const SupervisorLayout = ({ admin, onLogout, children }) => {
  ...
  // Payments sub-items (VIEW-ONLY for Supervisor - no Record Payment)
  const paymentsSubItems = [
    { path: '/supervisor/payments', icon: LayoutDashboard, label: 'Dashboard' },
    { path: '/supervisor/payments/invoices', icon: Receipt, label: 'Invoices' },
    { path: '/supervisor/payments/history', icon: Clock, label: 'Payment History' },
  ];
  ...
  const isPaymentsActive = paymentsSubItems.some(item => location.pathname === item.path) || location.pathname.startsWith('/supervisor/payments');
