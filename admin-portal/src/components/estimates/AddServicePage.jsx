...
const Field = ({ label, children }) => {
  ...
  return (
  <label className={sk("block min-w-0")}>
    <span className={sk("mb-2 block text-xs font-semibold text-slate-700")}>{label}</span>
    {children}
  </label>
  );
};
...
const Toggle = ({ label, checked, onChange }) => {
  ...
  return (
  <div>
    <span className={sk("mb-3 block text-xs font-semibold text-slate-700")}>{label}</span>
    <button type="button" role="switch" aria-label={label} aria-checked={checked} onClick={onChange}
      className={sk(`relative inline-flex h-6 w-11 items-center rounded-full transition focus-visible:outline focus-visible:outline-2 focus-visible:outline-blue-500 ${checked ? 'bg-blue-600' : 'bg-slate-300'}`)}>
      <span className={sk(`h-4 w-4 rounded-full bg-white shadow-sm transition-transform ${checked ? 'translate-x-6' : 'translate-x-1'}`)} />
    </button>
  </div>
  );
};
...
const AddServicePage = ({ admin, showToast, onBack, onSave, service, apiPath = '/api/admin/service-catalog', scoped = false, scopeLabel, embedded = false, leading }) => {
  ...
  if (admin?.role === 'operations_manager') {
    return <div className={sk("rounded-xl border bg-white p-6 text-sm text-slate-600")}>Service configuration is read-only for your role.</div>;
  }

  return (
    <form onSubmit={handleSubmit} className={sk("space-y-4 text-slate-900")}>
      {/* Header */}
      {/* One row, never wrapped: whatever the host supplies sits left, the actions right */}
      <header className={sk("flex items-center gap-3 overflow-x-auto")}>
        {leading && <div className={sk("mr-auto shrink-0")}>{leading}</div>}
        {!embedded && <div className={sk("mr-auto min-w-0")}>
          <h1 className={sk("truncate text-xl font-semibold")}>{service ? 'Edit Service' : 'Add Service'}{formData.pricingMethod ? ` — ${getFormulaText()}` : ''}</h1>
          <span className={sk("mt-1.5 inline-flex rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-medium text-slate-600")}>{scopeText}</span>
        </div>}
        ...
        <div className={sk("ml-auto flex shrink-0 items-center gap-3")}>
          <button type="button" onClick={onBack} disabled={isSubmitting} aria-label="Back to services" className={sk("rounded-lg border border-slate-200 bg-white p-2.5 shadow-sm transition hover:bg-slate-50")}>
            <ChevronLeft className={sk("h-5 w-5 text-slate-500")} />
          </button>
          <button type="button" onClick={onBack} disabled={isSubmitting} className={sk("rounded-lg border border-slate-200 bg-white px-5 py-2.5 text-sm font-medium text-blue-600 shadow-sm transition hover:bg-blue-50")}>Cancel</button>
          <button type="submit" disabled={isSubmitting || !formData.pricingMethod} className={sk("inline-flex items-center gap-2 rounded-lg bg-blue-600 px-5 py-2.5 text-sm font-medium text-white shadow-sm hover:bg-blue-700 disabled:opacity-50")}>
            {isSubmitting ? <Loader2 className={sk("h-4 w-4 animate-spin")} /> : <Save className={sk("h-4 w-4")} />}
            {isSubmitting ? 'Saving...' : 'Save Service'}
          </button>
        </div>
      </header>
      {formError && <div role="alert" className={sk("rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700")}>{formError}</div>}
      <fieldset disabled={isSubmitting} className={sk("min-w-0 space-y-5")}>
        ...
        <div className={sk("grid gap-5 xl:grid-cols-4")}>
          ...
          <section className={sk("overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm xl:col-span-3")}>
            ...
            <div className={sk("p-5 sm:p-6")}>
              ...
              <div className={sk("grid gap-5 sm:grid-cols-2")}>
                ...
                <div className={sk("sm:col-span-2")}>
                  <span className={sk("mb-3 block text-xs font-semibold text-slate-700")}>Pricing Method <span className={sk("text-red-500")}>*</span></span>
                  <div role="group" aria-label="Pricing Method" className={sk("grid grid-cols-2 gap-2 sm:grid-cols-3 2xl:grid-cols-6")}>
                    {PRICING_METHODS.map(method => (
                      <button key={method.value} type="button" aria-pressed={formData.pricingMethod === method.value}
                        ...
                        className={sk(`rounded-lg border px-3 py-2.5 text-xs font-semibold transition ${formData.pricingMethod === method.value
                    ...
                    {formData.pricingMethod && !PRICING_METHODS.some(method => method.value === formData.pricingMethod) && (
                      <button type="button" disabled aria-pressed="true" title="This method is no longer offered"
                        className={sk("cursor-not-allowed rounded-lg border border-amber-300 bg-amber-50 px-3 py-2.5 text-xs font-semibold text-amber-700")}>
            ...
            {isCapacitySlab && <div className={sk("border-t border-slate-100 p-5 sm:p-6")}>
              ...
              <button type="button" onClick={addCapacitySlab} disabled={capacitySlabs.length >= 100} className={sk("mt-3 inline-flex items-center justify-center gap-2 rounded-lg border border-blue-200 px-4 py-2.5 text-xs font-semibold text-blue-600 hover:bg-blue-50 disabled:opacity-50")}><Plus className={sk("h-4 w-4")} />Add Slab</button>
            </div>}
            {!formData.pricingMethod && <div className={sk("border-t border-slate-100 p-5 sm:p-6")}>
              <div className={sk("flex flex-col items-center gap-2 rounded-xl border border-dashed border-slate-300 bg-slate-50/70 px-6 py-10 text-center")}>
                <SlidersHorizontal className={sk("h-5 w-5 text-slate-400")} />
                <p className={sk("text-sm font-medium text-slate-700")}>No pricing method selected</p>
                <p className={sk("max-w-sm text-xs text-slate-500")}>Choose a method above to configure its rate, frequency, visits and markup.</p>
              </div>
            </div>}
            {formData.pricingMethod && <div className={sk("border-t border-slate-100 p-5 sm:p-6")}>
              <h2 className={sk("mb-5 text-sm font-semibold text-blue-600")}>{isRatePricing ? `${getFormulaText()} Configuration` : isCapacitySlab ? 'Fallback Frequency & Estimate Overrides' : 'Default Frequency'}</h2>
              <div className={sk(`grid gap-5 ${isRatePricing ? 'sm:grid-cols-2 lg:grid-cols-3' : 'sm:grid-cols-2'}`)}>
                ...
                <Field label="Default Frequency *"><select value={formData.defaultFrequency} onChange={event => changeFrequency(event.target.value)} className={sk(inputClass)}>{FREQUENCY_OPTIONS.map(frequency => <option key={frequency.value}>{frequency.value}</option>)}</select></Field>
              ...
              <div className={sk("mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3")}>
            ...
            {isVisitManpower && <section className={sk("border-t border-slate-100 p-5 sm:p-6")}>
              <h2 className={sk("text-sm font-semibold text-blue-600")}>Manpower Requirement Template (Optional)</h2>
              ...
              {manpowerRanges.length > 0 && <div className={sk("overflow-x-auto rounded-lg border border-slate-200")}>
                <table className={sk("w-full min-w-[900px] text-left text-xs")}>
                  ...
                  <tbody className={sk("divide-y divide-slate-100")}>{manpowerRanges.map((range, index) => <tr key={range.id}>
                    ...
                    <td className={sk("px-3 py-3")}><button type="button" aria-label={`Delete manpower range ${index + 1}`} onClick={() => setManpowerRanges(prev => prev.filter(item => item.id !== range.id))} className={sk("rounded p-2 text-slate-400 hover:bg-red-50 hover:text-red-600")}><Trash2 className={sk("h-4 w-4")} /></button></td>
                  </tr>)}</tbody>
                </table>
              </div>}
              <button type="button" onClick={addManpowerRange} disabled={manpowerRanges.length >= 100} className={sk("mt-3 inline-flex items-center gap-2 rounded-lg border border-blue-200 px-4 py-2.5 text-xs font-semibold text-blue-600 hover:bg-blue-50 disabled:opacity-50")}><Plus className={sk("h-4 w-4")} />Add Range</button>
            </section>}
            {/* 3. Markup & Margin */}
            {formData.pricingMethod && <div className={sk("border-t border-slate-100 p-5 sm:p-6")}>
              <h2 className={sk("mb-5 text-sm font-semibold text-blue-600")}>Default Markup</h2>
              {/* Markup is entered, then the margin and the customer price are calculated from it */}
              <div className={sk("grid gap-5 sm:grid-cols-2 lg:grid-cols-3")}>
                ...
                {!isCapacitySlab && <>
                  <Field label={`XLAND Margin (₹ per ${rateBasis})`}>
                    <input type="text" readOnly value={xlandRate == null ? '' : currency(xlandRate)}
                      className={sk(`${inputClass} bg-slate-50 text-slate-600`)} />
                  </Field>
                  <Field label={`Customer Price (₹ per ${rateBasis})`}>
                    <input type="text" readOnly value={customerRate == null ? '' : currency(customerRate)}
                      className={sk(`${inputClass} bg-slate-50 font-semibold text-slate-800`)} />
                  </Field>
                </>}
              </div>
            </div>}
          ...
          <aside className={sk("space-y-5")}>
            {/* Applicable Property Types */}
            <section className={sk("rounded-xl border border-slate-200 bg-white shadow-sm p-5")}>
              <h2 className={sk("mb-4 text-sm font-semibold")}>Applicable Property Types <span className={sk("text-red-500")}>*</span></h2>
              <div className={sk("grid grid-cols-2 gap-2 sm:grid-cols-3 xl:grid-cols-2")}>
                {PROPERTY_TYPES.map(type => (
                  <label key={type.id} className={sk(`flex cursor-pointer items-center gap-2 rounded-lg border p-2.5 transition ${formData.applicablePropertyTypes.includes(type.id) ? 'border-blue-200 bg-blue-50/50' : 'border-slate-200 hover:bg-slate-50'}`)}>
                    <input type="checkbox" checked={formData.applicablePropertyTypes.includes(type.id)} onChange={() => togglePropertyType(type.id)} className={sk("h-4 w-4 shrink-0 accent-blue-600")} />
                    <span className={sk("text-xs text-slate-700")}>{type.label}</span>
                  </label>
                ))}
                ...
                {formData.applicablePropertyTypes.filter(id => !PROPERTY_TYPES.some(type => type.id === id)).map(id => (
                  <label key={id} className={sk("flex cursor-pointer items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 p-2.5")} title="No longer offered for new services">
                    <input type="checkbox" checked onChange={() => togglePropertyType(id)} className={sk("h-4 w-4 shrink-0 accent-amber-600")} />
                    <span className={sk("text-xs text-amber-700")}>{propertyTypeLabel(id)}</span>
                  </label>
                ))}
              </div>
            </section>
            ...
            {/* Pricing Preview */}
            {isRatePricing && !isVisitManpower && <section className={sk("rounded-xl border border-slate-200 bg-white shadow-sm p-5")}>
              ...
              {!isFixedPrice && <Field label={`Total ${isQuantityBased ? 'Quantity' : isCapacityBased ? 'Capacity' : 'Area'} (${formData.unit})`}>
                <input inputMode="decimal" value={exampleAmountValue} onChange={event => setExampleAmount(event.target.value)} className={sk(inputClass)} />
              </Field>}
              <dl className={sk(`space-y-3 text-xs text-slate-600 ${isFixedPrice ? '' : 'mt-4'}`)}>
                <div className={sk("flex justify-between gap-3")}><dt>{isFixedPrice ? 'Fixed Rate per Visit' : `Rate per ${formData.unit} per Visit`}</dt><dd className={sk("font-medium text-slate-800")}>{currency(formData[rateField] === '' ? null : Number(formData[rateField]))}</dd></div>
                <div className={sk("flex justify-between gap-3")}><dt>Visits Per Year</dt><dd className={sk("font-medium text-slate-800")}>{formData.defaultVisitsPerYear}</dd></div>
                <div className={sk("flex justify-between gap-3 border-t border-slate-100 pt-3")}><dt className={sk("font-semibold")}>Annual Vendor Cost</dt><dd className={sk("font-semibold text-slate-800")}>{currency(examplePricing?.vendorCost)}</dd></div>
                <div className={sk("flex justify-between gap-3")}><dt>Default Markup</dt><dd className={sk("font-medium text-slate-800")}>{formData.defaultMarkupPercentage === '' ? '—' : `${formData.defaultMarkupPercentage}%`}</dd></div>
                <div className={sk("flex justify-between gap-3")}><dt>XLAND Margin</dt><dd className={sk("font-medium text-slate-800")}>{currency(examplePricing?.xlandCost)}</dd></div>
                <div className={sk("flex justify-between gap-3")}><dt className={sk("font-semibold")}>Customer Price</dt><dd className={sk("font-semibold text-blue-600")}>{currency(examplePricing?.customerPrice)}</dd></div>
                <div className={sk("flex justify-between gap-3")}><dt>Margin</dt><dd className={sk("font-medium text-slate-800")}>{examplePricing?.marginPercentage == null ? '—' : `${examplePricing.marginPercentage.toFixed(2)}%`}</dd></div>
              </dl>
              {!validExampleAmount && <p className={sk("mt-3 text-xs text-amber-700")}>Enter {isQuantityBased ? 'a quantity' : isCapacityBased ? 'a capacity' : 'an area'} greater than zero to see the pricing.</p>}
            </section>}
            {isVisitManpower && <section className={sk("rounded-xl border border-slate-200 bg-white shadow-sm p-5")}>
              <h2 className={sk("mb-4 text-sm font-semibold")}>Pricing Preview</h2>
              <div className={sk("space-y-3")}>
                {manpowerRanges.length > 0 && <Field label="Property Area (Sq Ft)"><input inputMode="numeric" value={exampleManpowerArea} onChange={event => { setExampleManpowerArea(event.target.value); setExamplePersonnel(String(suggestedManpower(manpowerConfig, event.target.value))); }} className={sk(inputClass)} /></Field>}
                <Field label="Manpower (Persons)"><input inputMode="numeric" value={examplePersonnel} onChange={event => setExamplePersonnel(event.target.value)} className={sk(inputClass)} /></Field>
                {formData.overtimeRatePerHour !== '' && <Field label="Overtime Hours per Person / Visit"><input inputMode="decimal" value={exampleOvertime} onChange={event => setExampleOvertime(event.target.value)} className={sk(inputClass)} /></Field>}
              </div>
              <dl className={sk("mt-4 space-y-3 text-xs text-slate-600")}>
                {manpowerRanges.length > 0 && <div className={sk("flex justify-between gap-3")}><dt>Matching Area Range</dt><dd className={sk("font-medium text-slate-800")}>{manpowerRangeLabel(manpowerExample?.range)}</dd></div>}
                <div className={sk("flex justify-between gap-3")}><dt>Rate per Person per Visit</dt><dd className={sk("font-medium text-slate-800")}>{currency(manpowerExample?.ratePerPerson)}</dd></div>
                <div className={sk("flex justify-between gap-3")}><dt>Visits Per Year</dt><dd className={sk("font-medium text-slate-800")}>{formData.defaultVisitsPerYear}</dd></div>
                <div className={sk("flex justify-between gap-3 border-t border-slate-100 pt-3")}><dt className={sk("font-semibold")}>Annual Vendor Cost</dt><dd className={sk("font-semibold text-slate-800")}>{currency(manpowerExample?.vendorCost)}</dd></div>
                <div className={sk("flex justify-between gap-3")}><dt>Default Markup</dt><dd className={sk("font-medium text-slate-800")}>{formData.defaultMarkupPercentage === '' ? '—' : `${formData.defaultMarkupPercentage}%`}</dd></div>
                <div className={sk("flex justify-between gap-3")}><dt>XLAND Margin</dt><dd className={sk("font-medium text-slate-800")}>{currency(manpowerExample?.customerPrice == null ? null : manpowerExample.customerPrice - manpowerExample.vendorCost)}</dd></div>
                <div className={sk("flex justify-between gap-3")}><dt className={sk("font-semibold")}>Customer Price</dt><dd className={sk("font-semibold text-blue-600")}>{currency(manpowerExample?.customerPrice)}</dd></div>
                <div className={sk("flex justify-between gap-3")}><dt>Margin</dt><dd className={sk("font-medium text-slate-800")}>{manpowerExample?.marginPercentage == null ? '—' : `${manpowerExample.marginPercentage.toFixed(2)}%`}</dd></div>
              </dl>
              {manpowerExample?.error && <p className={sk("mt-3 text-xs text-amber-700")}>{manpowerExample.error}</p>}
            </section>}
            {isCapacitySlab && <section className={sk("rounded-xl border border-slate-200 bg-white shadow-sm p-5")}>
              ...
              <dl className={sk("mt-4 space-y-3 text-xs text-slate-600")}>
                ...
                <div className={sk("flex justify-between gap-3")}><dt>Frequency</dt><dd className={sk("font-medium text-slate-800")}>{previewSlab?.defaultFrequency ?? '—'}</dd></div>
                <div className={sk("flex justify-between gap-3")}><dt>Visits Per Year</dt><dd className={sk("font-medium text-slate-800")}>{previewSlab?.defaultVisitsPerYear ?? '—'}</dd></div>
                <div className={sk("flex justify-between gap-3 border-t border-slate-100 pt-3")}><dt className={sk("font-semibold")}>Annual Vendor Cost</dt><dd className={sk("font-semibold text-slate-800")}>{currency(slabVendorCost)}</dd></div>
                <div className={sk("flex justify-between gap-3")}><dt>Default Markup</dt><dd className={sk("font-medium text-slate-800")}>{formData.defaultMarkupPercentage === '' ? '—' : `${formData.defaultMarkupPercentage}%`}</dd></div>
                <div className={sk("flex justify-between gap-3")}><dt>XLAND Margin</dt><dd className={sk("font-medium text-slate-800")}>{currency(slabXlandCost)}</dd></div>
                <div className={sk("flex justify-between gap-3")}><dt className={sk("font-semibold")}>Customer Price</dt><dd className={sk("font-semibold text-blue-600")}>{currency(slabCustomerPrice)}</dd></div>
                <div className={sk("flex justify-between gap-3")}><dt>Margin</dt><dd className={sk("font-medium text-slate-800")}>{slabCustomerPrice == null ? '—' : `${(slabXlandCost / slabCustomerPrice * 100).toFixed(2)}%`}</dd></div>
              </dl>
              {slabPreviewMessage && <p className={sk("mt-3 text-xs text-amber-700")}>{slabPreviewMessage}</p>}
            </section>}
          </aside>
        </div>
        {/* 2. Monthly Manpower Configuration, kept for services saved before the per-visit basis */}
        {formData.pricingMethod === 'manpower' && !isVisitManpower && <section className={sk("rounded-xl border border-slate-200 bg-white shadow-sm p-5 sm:p-6")}>
          <h2 className={sk("mb-5 text-sm font-semibold")}>{getFormulaText()} Configuration</h2>
          <div className={sk("grid gap-5 sm:grid-cols-2 lg:grid-cols-3")}>
            <Field label={`Monthly Vendor Rate (₹) per ${formData.unit} *`}>{numberInput('monthlyRate')}</Field>
            <Field label="Billing Period *"><select value={formData.billingPeriod} onChange={event => setField('billingPeriod', event.target.value)} className={sk(inputClass)}>{BILLING_PERIODS.map(period => <option key={period}>{period}</option>)}</select></Field>
