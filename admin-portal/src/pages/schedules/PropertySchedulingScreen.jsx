...
const PropertySchedulingScreen = ({ user, portalType = 'admin' }) => {
  ...
  // Anything else (a click event, when used straight as an onClick handler) falls
  // back to the planned visits in state.
  const handlePrepareConfirmation = (directVisits = null) => {
    if (!selectedService) return;
    
    const visitsToUse = Array.isArray(directVisits) ? directVisits : plannedVisits;
          ...
          <div className="flex-1 min-w-0">
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-7 gap-3 sm:gap-4">
              <div className="min-w-0">
                ...
                <p className="text-xs sm:text-sm font-semibold text-blue-600 truncate">{property?.propertyId || '-'}</p>
              </div>
              <div className="min-w-0">
                <p className="text-[10px] text-gray-400 uppercase tracking-wide">Property Name</p>
                <p className="text-xs sm:text-sm font-semibold text-gray-900 truncate">{property?.propertyName || '-'}</p>
              </div>
              <div className="min-w-0 col-span-2 sm:col-span-1">
                ...
                <span className="text-[10px] sm:text-xs text-gray-500 font-normal flex items-center gap-1 mt-0.5"><Phone className="w-3 h-3" /> {property?.customerPhone || '-'}</span>
              </div>
              <div className="min-w-0">
                <p className="text-[10px] text-gray-400 uppercase tracking-wide">Property Type</p>
                <p className="text-xs sm:text-sm font-medium text-gray-700 flex items-center gap-1 truncate"><Building2 className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-gray-400 flex-shrink-0" /> {property?.propertyType || '-'}</p>
              </div>
              <div className="min-w-0">
                <p className="text-[10px] text-gray-400 uppercase tracking-wide">Zone</p>
                <span className="inline-flex px-1.5 sm:px-2 py-0.5 bg-blue-50 text-blue-700 text-[10px] sm:text-xs font-medium rounded border border-blue-200">{getZoneName(property?.zone) || '-'}</span>
              </div>
              <div className="min-w-0">
                <p className="text-[10px] text-gray-400 uppercase tracking-wide">Package</p>
                <p className="text-xs sm:text-sm font-medium text-purple-700 truncate">{property?.packageName || '-'}</p>
              </div>
            </div>
          </div>
        </div>
