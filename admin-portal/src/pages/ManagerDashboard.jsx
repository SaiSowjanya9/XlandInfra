  ...
  const fetchDashboardData = useCallback(async (isInitialLoad = false) => {
    ...
    try {
      ...
      const [dashboardRes, estimatesRes, workOrdersRes, propertiesRes, invoicesRes, vendorsRes, employeesRes] = await Promise.all([
        ...
        fetch(`${API_BASE}/api/manager/estimates`, {
          headers: { 'Authorization': `Bearer ${token}` }
        }),
        fetch(`${API_BASE}/api/manager/work-orders`, {
          headers: { 'Authorization': `Bearer ${token}` }
        }),
        fetch(`${API_BASE}/api/manager/properties`, {
          headers: { 'Authorization': `Bearer ${token}` }
        }),
  ...
  const getFilteredProperties = () => {
    ...
    switch (propertyChartFilter) {
      ...
      case 'sixmonths':
        ...
        break;
      case 'year':
        filterStart.setMonth(0, 1); filterStart.setHours(0,0,0,0);
        break;
      default:
  ...
  const propertyTypeData = (() => {
    ...
    return Object.entries(typeCounts)
      .map(([name, value]) => ({ 
        name, 
        value, 
        color: PROPERTY_TYPE_COLORS[name] || '#78716C' 
      }))
      .sort((a, b) => b.value - a.value);
  })();
      ...
      <div className="flex flex-col gap-4 overflow-visible">
        <div className="flex flex-col gap-4">
          ...
          <DashboardStatCards cards={[
            ...
            { label: 'Employees', value: realTimeStats.employees, to: '/manager/employees/zones', icon: Users, iconBg: 'bg-orange-50', iconColor: 'text-orange-600', hoverBorder: 'hover:border-orange-200' },
            { label: 'Work Orders', value: realTimeStats.workOrders, to: '/manager/work-orders', icon: ClipboardList, iconBg: 'bg-purple-50', iconColor: 'text-purple-600', hoverBorder: 'hover:border-purple-200' },
            { label: 'Estimates', value: realTimeStats.estimates, to: '/manager/estimates', icon: FileText, iconBg: 'bg-teal-50', iconColor: 'text-teal-600', hoverBorder: 'hover:border-teal-200' },
            ...(pendingSchedulesCount > 0 ? [{ label: 'Pending Schedules', value: pendingSchedulesCount, to: '/manager/schedules/pending', icon: Calendar, iconBg: 'bg-orange-500', iconColor: 'text-white', accent: true }] : [])
          ]} />
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to="/manager/properties" className="text-sm text-blue-600 hover:text-blue-700 font-medium flex items-center gap-1">
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-6 space-y-6">
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/manager/work-orders" className="text-sm text-blue-600 hover:text-blue-700 font-medium flex items-center gap-1">
              View All <ArrowRight className="w-4 h-4" />
            </Link>
          </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/manager/estimates" className="text-sm text-blue-600 hover:text-blue-700 font-medium flex items-center gap-1">
              View All <ArrowRight className="w-4 h-4" />
            </Link>
          </div>
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                ...
                <p className="text-xs text-gray-500">Employee zone management</p>
              </div>
            </div>
            <ArrowRight className="w-4 h-4 text-gray-400 group-hover:text-gray-600 transition-colors" />
          </button>
