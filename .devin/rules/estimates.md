...
- **There is exactly one Add Service button, and it sits in the bar below the table**, not in the Action column — that column is 5% wide and clipped the label in half. It commits the entry row and leaves a fresh empty row; the Action column holds only the remove action for committed rows, and the Total Custom Services footer appears once there is a row. Enter in any entry field commits too.
- The button is **disabled until the row has a name and a customer price**, so a blank service cannot be added. It must read as deliberately disabled (grey, not faded blue), and the bar beside it says why. Do not remove the guard to make it look active.
