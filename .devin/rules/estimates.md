...
- **In package mode the configured-service dropdown sits on the structure row beside the package dropdown**, so both ways of putting a service on the estimate are chosen in one place. `ServiceCatalogPicker` takes `inline` for this: it drops the blue panel and the `max-w-md` cap and matches the package dropdown's grey-bordered style. Custom mode leaves the picker in its own panel below, unchanged. Wired in FP; the other portals still pass only the package dropdown. A form's package dropdown must be stacked (label above select) or it will not line up with the picker beside it.
...
- **Build Custom Services shows `components/estimates/CustomServicesTable.jsx`**: committed rows plus a permanently empty last row, which is where a service is entered. Columns are `#`, Service, Input / Details, Frequency, Visits / Year, Customer Price (₹), Action. Deliberately absent, per the reference screenshots: Import Services, Method, Vendor Cost and Margin.
- **There is exactly one Add Service button, and it sits in the bar below the table**, not in the Action column — that column was too narrow and clipped the label in half. It commits the entry row and leaves a fresh empty row, and the Total Custom Services footer appears once there is a row. Enter in any entry field commits too.
- A committed row carries **edit and remove**. Edit turns that row into the same six fields as the entry row with confirm/cancel in its Action cell, and keeps the row's `addonId` so it stays in place; cancel restores what was there. A half-finished edit is held locally and never reaches the estimate.
- The entry row is marked with a **plus in the `#` column, not the next row number** — a number read as a service that had already been added, which is why its empty Action cell kept being reported as missing edit and delete.
