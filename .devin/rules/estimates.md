...
- **A Quantity Based service settles two things per estimate: Category and Vendor Required (Yes/No).** They appear in the pricing dialog for that method only, seeded from the service and saved with the estimate row. Every other field of a catalog row is rebuilt from the catalog on save — that is what stops a client altering a saved service — so these two are the only values read back from the request, through `catalogEstimateOverrides` in `backend/utils/estimateData.js`, which validates them and is applied in both the FP and Manager estimate validators. Adding a third override means changing that function, not the routes.
